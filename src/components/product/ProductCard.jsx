...
import { FaFacebookF, FaTwitter, FaRegStar, FaStar, FaStarHalfAlt } from "react-icons/fa"
            ...
            <div className="product card">
                <div className="images w-full">
                    {images.slice(0, 1).map((cover, index) => {
                        return <img key={"img-"+index} src={cover?.image} alt={description}
                ...
                <div className="deatils flex items-center flex-col bg-white pt-6">
                    ...
                    <div className="flex items-center gap-3">
                        {price.slice(0, 1).map((priceItem, index)=>(
                                <div className="flex gap-3" key={"price-"+index}>
                                    <BodyOne className="line-through" key={"price-"+index}>
                                        ${priceItem.value}
                                    </BodyOne>
                        ...
                        <div className="modal-overlay" onClick={closeModal}>
                            ...
                            >
                                ...
                                <div className="modal-details w-1/2 h-[500px] overflow-y-scroll p-9">
                                    ...
                                    {price.slice(0, 1).map((priceItem, index)=>(
                                            <div className="flex items-center gap-3" key={"price-"+index}>
                                                <BodyOne className="line-through mt-4" key={"price-"+index}>
                                                    ${priceItem.value}
                                                </BodyOne>
                                    ...
                                    <button className="close-btn absolute top-0 right-0 w-12 h-12 flex justify-center items-center bg-primary-green text-white" onClick={closeModal}>
                                        <IoClose size={20} />
                                    </button>
                                </div>
                            </div>
